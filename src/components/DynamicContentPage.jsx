...
export default function DynamicContentPage({ contentId, setView, previousView = 'landing', currentUser, userRole, setSelectedUserId }) {
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 flex-col gap-4">
        ...
        <button onClick={() => setView(previousView)} className="px-6 py-2 bg-[#990000] text-white rounded-lg cursor-pointer">Geri Dön</button>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#f8f9fc] animate-fade-in pb-32">
      {/* Hero Banner */}
      <div className="relative h-[400px] w-full overflow-hidden bg-[#990000]">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 h-full flex flex-col justify-center">
          <button 
            ...
            className="w-10 h-10 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/30 text-white flex items-center justify-center transition cursor-pointer mb-8 shadow-xs"
            title="Geri Dön"
          >
            <ArrowLeft size={18} />
          </button>
              ...
              return (
                <div key={idx} className="mb-12 last:mb-0">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-red-50 text-[#990000] flex items-center justify-center flex-shrink-0">
            ...
            {data.contactInfo && (
              <div className="bg-white rounded-xl shadow-xl shadow-black/5 p-8 border border-gray-100">
                <h3 className="text-lg font-black text-gray-900 mb-6 flex items-center gap-2">
                  <Icons.PhoneCall size={20} className="text-[#990000]" /> İletişim Bilgileri
                </h3>
                <div className="flex flex-col gap-5">
                  <div className="flex items-start gap-3">
                    <div className="mt-1 w-8 h-8 rounded-full bg-red-50 text-[#990000] flex items-center justify-center flex-shrink-0"><Icons.Mail size={14} /></div>
                    <div>
