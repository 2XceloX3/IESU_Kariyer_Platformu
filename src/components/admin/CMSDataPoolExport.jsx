...
export default function CMSDataPoolExport() {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-gradient-to-r from-slate-950 via-red-950 to-indigo-950 text-white rounded-3xl p-6 md:p-8 shadow-xl border border-red-900/80 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 shrink-0">
            <div className="bg-red-950/90 border border-red-900 p-3.5 rounded-2xl">
            ...
            <div className="bg-red-950/90 border border-red-900 p-3.5 rounded-2xl col-span-2 sm:col-span-1">
        ...
        <div className="flex flex-wrap gap-2.5 mt-8 border-t border-red-900/80 pt-6">
          <button 
            onClick={() => { setSubTab('bmi'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'bmi' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
            <Activity size={15} /> BMI & Sağlık ({bmiRecords.length})
          </button>
          <button 
            onClick={() => { setSubTab('helpdesk'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'helpdesk' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
            <Server size={15} /> BİDB Destek Biletleri ({helpdeskTickets.length})
          </button>
          <button 
            onClick={() => { setSubTab('clubs'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'clubs' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
            <Users size={15} /> Kulüp Başvuruları ({clubApplications.length})
          </button>
          <button 
            onClick={() => { setSubTab('labs'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'labs' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
            <FlaskConical size={15} /> Ar-Ge Lab Rezervasyonları ({labReservations.length})
          </button>
          <button 
            onClick={() => { setSubTab('events'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'events' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
            <Ticket size={15} /> Etkinlik Bilet Kayıtları ({eventRegistrations.length})
          </button>
          <button 
            onClick={() => { setSubTab('surveys'); setSearch(''); }}
            className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 ${subTab === 'surveys' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-red-950/80 text-slate-400 hover:text-white hover:bg-red-900'}`}
          >
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-red-900">
                  {helpdeskTickets.filter(t => t.name.toLowerCase().includes(search.toLowerCase()) || t.subject.toLowerCase().includes(search.toLowerCase())).map(t => (
                    <tr key={t.id} className="hover:bg-slate-50 transition">
                      <td className="p-3.5 font-mono text-red-600 font-black">{t.id}</td>
                      <td className="p-3.5 font-bold text-red-950">{t.name}</td>
                      <td className="p-3.5 text-slate-600 font-medium">{t.email}</td>
                      <td className="p-3.5 font-bold text-red-950">{t.subject}</td>
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-red-900">
                  {clubApplications.filter(c => c.name.toLowerCase().includes(search.toLowerCase()) || c.clubName.toLowerCase().includes(search.toLowerCase())).map(c => (
                    <tr key={c.id} className="hover:bg-slate-50 transition">
                      <td className="p-3.5 font-mono text-red-600 font-black">{c.id}</td>
                      <td className="p-3.5 font-bold text-red-950">{c.name}</td>
                      <td className="p-3.5 text-red-600 font-bold">{c.clubName}</td>
