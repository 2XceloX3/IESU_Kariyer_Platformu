...
export default function CMSAlumniAssoc({ posts = [], setPosts, currentUser }) {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-3xl border border-red-100 shadow-[0_8px_30px_rgb(185,28,28,0.05)]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 space-y-5">
              <form id="assoc-post-form" onSubmit={handleAdd} className="space-y-5">
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">Başlık</label>
                  <input type="text" required value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Örn: 2026 Mezunlar Buluşması" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all font-medium text-gray-900 placeholder:text-gray-400" />
                </div>
                ...
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">Afiş/Görsel Yükle</label>
                  {!form.imageUrl ? (
                    <label className="w-full bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl px-4 py-8 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-100 hover:border-red-300 transition-all group">
                      <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-3 shadow-sm group-hover:scale-110 transition-transform">
                        <ImagePlus size={24} className="text-gray-400 group-hover:text-red-500" />
                      </div>
                      <span className="text-sm font-bold text-gray-700">Görsel seçmek için tıklayın</span>
                      <span className="text-xs font-medium text-gray-400 mt-1">PNG, JPG veya WEBP (Maks 5MB)</span>
                ...
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">İçerik Metni</label>
                  <textarea required value={form.content} onChange={e => setForm({...form, content: e.target.value})} rows="5" placeholder="İçerik detaylarını buraya yazın..." className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all font-medium text-gray-900 placeholder:text-gray-400 resize-none"></textarea>
                </div>
              </form>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="sticky top-24">
                ...
                <div className="flex gap-3 mt-6">
                  <button type="submit" form="assoc-post-form" className="flex-1 bg-iesu-red text-white py-3 rounded-xl font-bold text-sm hover:bg-red-700 shadow-[0_4px_12px_rgb(185,28,28,0.2)] transition-all active:scale-95 flex items-center justify-center gap-2">
