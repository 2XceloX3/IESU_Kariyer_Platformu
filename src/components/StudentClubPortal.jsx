...
import { 
  ...
  Wallet, ShieldCheck, MapPin, Activity, ArrowLeft, X
} from 'lucide-react';
...
export default function StudentClubPortal({ currentUser, clubs, setClubs, setView, previousView, clubApplications, setClubApplications }) {
  ...
  const handleEventSubmit = (e) => {
    ...
    alert("Etkinlik başvurunuz başarıyla dekanlığa iletildi!");
  };
              ...
              {myManagedClubs.length > 0 ? myManagedClubs.map(club => (
                <div key={club.id} className="bg-white rounded-3xl p-6 border border-blue-100 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setShowEventModal(true)} className="flex-1 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-700 font-bold rounded-xl transition text-sm">Etkinlik Oluştur</button>
                  </div>
                </div>
              )) : (
