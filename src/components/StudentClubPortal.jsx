...
import { 
  ...
  Wallet, ShieldCheck, MapPin, Activity, ArrowLeft
} from 'lucide-react';
import Logo from './Logo';

export default function StudentClubPortal({ currentUser, clubs, setClubs, setView, previousView }) {
  ...
  const myManagedClubs = (clubs || []).filter(c => c.presidentId === currentUser?.id || (c.admins || []).includes(currentUser?.id));
  const myJoinedClubs = (clubs || []).filter(c => (c.members || []).some(m => m.id === currentUser?.id));
  ...
  const filteredClubs = (clubs || []).filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()) || c.description?.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleApplyClub = () => {
    alert("Kulüp kurma başvurusu formuna yönlendiriliyorsunuz...");
  };
        ...
        <div className="max-w-5xl mx-auto mt-8 px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-6 text-white shadow-md">
               ...
               <button className="w-full py-3 bg-white text-blue-900 font-black rounded-xl hover:bg-blue-50 transition">Üyelik Talebi Gönder</button>
            </div>
    ...
    <div className="min-h-screen bg-[#F8F9FC] font-sans">
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-40">
        ...
        <div className="max-w-7xl mx-auto px-6 flex gap-6">
          ...
          <button onClick={() => setActiveTab('my_clubs')} className={`pb-4 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === 'my_clubs' ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-900'}`}>Kulüplerim</button>
          {(isAdmin || isDean) && (
            <button onClick={() => setActiveTab('admin')} className={`pb-4 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === 'admin' ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-900'}`}>Dekanlık Onayları</button>
          )}
        </div>
      </nav>
        ...
        {activeTab === 'my_clubs' && (
          <div className="animate-fade-in space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {myManagedClubs.length > 0 ? myManagedClubs.map(club => (
                <div key={club.id} className="bg-white rounded-3xl p-6 border border-blue-100 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">2 Yeni Üyelik Talebi</p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button className="flex-1 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-700 font-bold rounded-xl transition text-sm">Etkinlik Oluştur</button>
                  </div>
                </div>
              )) : (
                <div className="col-span-full bg-white p-8 rounded-3xl border border-dashed border-gray-200 text-center">
                   <p className="text-gray-500 mb-4">Şu an yönettiğiniz bir kulüp bulunmuyor.</p>
                   <button onClick={handleApplyClub} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700">Kulüp Kurma Başvurusu Yap</button>
                </div>
              )}
            </div>
          </div>
                 ...
                 {applications.map(app => (
                   <div key={app.id} className="flex flex-col md:flex-row justify-between md:items-center gap-4 p-5 rounded-2xl border border-gray-100 hover:border-blue-100 hover:shadow-md transition bg-white">
                     ...
                     <div className="flex items-center gap-2">
                       {app.status === 'pending' ? (
                         <>
                           <button className="px-4 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold rounded-lg transition text-sm flex items-center gap-1"><CheckCircle2 size={16} /> Onayla</button>
                           <button className="px-4 py-2 bg-red-50 text-red-700 hover:bg-red-100 font-bold rounded-lg transition text-sm flex items-center gap-1"><XCircle size={16} /> Reddet</button>
                         </>
                       ) : (
